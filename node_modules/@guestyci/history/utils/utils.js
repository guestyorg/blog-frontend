...
var _createHistory = require("../createHistory/createHistory");

var _routeListener = require("../routerListener/routeListener");
...
function getQueryParams() {
  var search = (0, _createHistory.getHistory)().location.search.replace('?', '');
...
function _buildSearchQueryParam() {
  ...
  var newSearch = '';
  var isParamsEmpty = (0, _isEmpty["default"])(params);
  var search = (0, _createHistory.getHistory)().location.search.replace('?', '');

  if (preserveQueryParams && isParamsEmpty) {
    newSearch = search;
  } else if (preserveQueryParams && !isParamsEmpty) {
    var newQuery = (0, _extends2["default"])({}, params, getQueryParams());
    newSearch = _buildURLQueryParams(newQuery);
  } else if (!preserveQueryParams && !isParamsEmpty) {
    newSearch = _buildURLQueryParams(params);
  }

  return !(0, _isEmpty["default"])(newSearch) ? "?".concat(newSearch) : newSearch;
}
/**
...
function getPathName() {
  ...
  try {
    return relative ? absWindow.location.pathname.replace((0, _createHistory.getHistory)().baseUrl, '') : absWindow.location.pathname;
  } catch (e) {
    return e;
  }
}
...
function link(path) {
  ...
  } else if ((0, _createHistory.getHistory)().isFragment) {
    (0, _routeListener.postRouteChangeOutOfApp)(path);
  }
}
/**
...
function setQueryParams(params) {
  ...
  (0, _createHistory.getHistory)().push({
    search: search
  });
}
/**
...
function go(pathname) {
  ...
  if (!(0, _isEmpty["default"])(pathname)) {
    var search;

    if ((0, _isString["default"])(params)) {
      search = params;
    } else {
...
function replace(_ref) {
  var pathname = _ref.pathname,
      search = _ref.search,
      rest = (0, _objectWithoutProperties2["default"])(_ref, ["pathname", "search"]);
  var formattedSearch = (0, _isString["default"])(search) ? search : _buildSearchQueryParam(search);
  (0, _createHistory.getHistory)().replace((0, _extends2["default"])({
    pathname: pathname,
...
function push(payload, preserveQueryParams) {
  if ((0, _isString["default"])(payload)) {
    go(payload);
  } else {
...
function location() {
  return (0, _createHistory.getHistory)().location;
}

function createHref(href) {
  return (0, _createHistory.getHistory)().createHref(href);
}

function length() {
  return (0, _createHistory.getHistory)().length;
}

var _default = {
  listen: (0, _createHistory.getHistory)().listen,
  goBack: (0, _createHistory.getHistory)().goBack,
  goForward: (0, _createHistory.getHistory)().goForward,
  location: location,
  go: go,
