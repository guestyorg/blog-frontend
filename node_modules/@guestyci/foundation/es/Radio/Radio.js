...
const Radio = React.forwardRef(function Radio(_ref, ref) {
  let {
    ...
    jss
      ...
      props = _objectWithoutPropertiesLoose(_ref, ["id", "checked", "onChange", "disabled", "value", "name", "children", "style", "className", "jss"]);

  const classes = useStyles({
    jss
  });
  const [isHovering, hoverRef] = useHover();
  const radioData = useRadioGroup();
  const checked = radioData.value ? radioData.value === value : checkedProp;
  const onChange = radioData.onChange || onChangeProp;
  const name = radioData.name || nameProp;
