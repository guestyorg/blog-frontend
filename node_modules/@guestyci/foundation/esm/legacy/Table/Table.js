...
var Table = React.forwardRef(function Table(_ref2, ref) {
  ...
  var cachedCells = useRef(null);

  var _useTranslations = useTranslations(['smartviews_v3:table.table.empty_state_title', 'We searched everywhere']),
      _useTranslations2 = _slicedToArray(_useTranslations, 1),
      emptyStateTitle = _useTranslations2[0];

  var _useTranslations3 = useTranslations(['smartviews_v3:table.table.empty_state_description', 'And couldn’t find anything. Try again?']),
      _useTranslations4 = _slicedToArray(_useTranslations3, 1),
      emptyStateDescription = _useTranslations4[0];

  var scrollableRef = useInfiniteScroll(_extends({}, infiniteScrollOptions, {
    isLoading: isLoading,
    pageSize: pageSize
  }));
  ...
  var handleSelectRowChange = useCallback(function (rowId) {
    onCheckedRowsChange({
      rowId: rowId
    });
  }, [onCheckedRowsChange]);
  var handleMasterCheckboxChange = useCallback(function () {
    onCheckedRowsChange({
      selectAll: true
    });
  }, [onCheckedRowsChange]);
  var handleColumnResize = useCallback(function () {
    cachedCells.current = null;
    onColumnResize.apply(void 0, arguments);
  }, [onColumnResize]);
  var handleColumnReorderChange = useCallback(function (_ref3) {
    var sourceData = _ref3.sourceData,
        targetData = _ref3.targetData;
    cachedCells.current = null;

    var columns = _toConsumableArray(children);

    if (sourceData !== targetData) {
      var orderedColumns = getOrderedColumnsById(columns, sourceData, targetData);
      onColumnOrderChange(orderedColumns);
    }
  }, [onColumnOrderChange, children]);
  var handleHeaderSort = useCallback(function (args) {
  ...
  var CellRowComponent = rowComponent || CellRow;
  ...
  var _useHorizontalScrollS = useHorizontalScrollState([canRenderScrollShadows, scrollableRef]),
      _useHorizontalScrollS2 = _slicedToArray(_useHorizontalScrollS, 3),
