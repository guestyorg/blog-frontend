...
var hasActiveDates = function hasActiveDates(activeDates) {
  return !isEmpty(activeDates) && activeDates.start && activeDates.end;
};
/**
...
export var toISODateString = function toISODateString(d) {
...
export var subtractDay = function subtractDay(day) {
  return new Date(day.getTime() - 86400000);
};
export var addDay = function addDay(day) {
  return new Date(day.getTime() + 86400000);
};
/**
...
export var previousDayStatus = function previousDayStatus(calendar, day) {
  var prevStatus = {};
...
export var nextDayStatus = function nextDayStatus(calendar, day) {
  var nextStatus = {};
...
export var getGuestDetails = function getGuestDetails(reservation) {
  return {
...
export var getReservationTitle = function getReservationTitle(code, source, isOwner, isSR) {
  if (isOwner) {
    return 'Owner stay';
  }
...
export var getCurrencySymbol = function getCurrencySymbol(currency, amount) {
...
export var getBlockType = function getBlockType(blocks) {
  var bd = blocks.bd,
      abl = blocks.abl,
      sr = blocks.sr,
      bw = blocks.bw,
...
export var isSameBlock = function isSameBlock(block1, block2) {
...
export var isBlockReserved = function isBlockReserved(blocks) {
  return blocks && Object.keys(blocks).some(function (key) {
    return blocks[key] && reservedBlocks.has(key);
  });
};
export var hasOnlyPTBlock = function hasOnlyPTBlock(blocks) {
...
export var isLastDay = function isLastDay(prevId, nowId) {
  return prevId && nowId !== prevId;
};
export var isFirstDay = function isFirstDay(prevId, nowId, nextId) {
  return nowId && nextId && nextId === nowId && nowId !== prevId;
};
export var checkReservationDetails = function checkReservationDetails(calendar, day, activeDates) {
  var cost;
  var type;
  var guest;
  var block;
  ...
  if (!isEmpty(calObj)) {
    ...
    if (hasActiveDates(activeDates) && moment(day).startOf('day').diff(activeDates.start, 'days') >= 0 && moment(activeDates.end).diff(day, 'days') >= 0) {
      var activeFirstDay = moment(day).startOf('day').diff(activeDates.start, 'days') === 0;
      var activeLastDay = moment(activeDates.end).endOf('day').diff(day, 'days') === 0;
...
export var stringToDate = function stringToDate(stringDate) {
  return moment(stringDate).format('MM/D/YY, h:mm A');
};
/**
...
export var colorFromStatus = function colorFromStatus(status) {
  switch (status) {
...
export var isInvalidPlannedDate = function isInvalidPlannedDate(d1, d2) {
...
export var getStayDateTime = function getStayDateTime(date, time) {
  var dateString = date;
...
export var formatCalendarDay = function formatCalendarDay(day) {
...
export var formatCalendarMonth = function formatCalendarMonth(day) {
  var month = day.getMonth();
...
export var makeTitleCase = function makeTitleCase() {
